...
.park-map .pennant{transform-box:fill-box;transform-origin:0 50%;
  animation:wave 1.3s ease-in-out infinite alternate;animation-delay:calc(var(--fd) * -1)}
@keyframes wave{from{transform:skewY(-10deg) scaleX(.84)}to{transform:skewY(8deg) scaleX(1.06)}}
.park-map .tw{transform-box:fill-box;transform-origin:50% 50%;
  animation:twinkle 3.2s ease-in-out infinite;animation-delay:calc(var(--tw) * -1)}
@keyframes twinkle{0%,100%{transform:scale(.2) rotate(0deg);opacity:.15}50%{transform:scale(1) rotate(90deg);opacity:1}}
.park-map .birds{animation:fly 34s linear infinite}
...
.park-map .wing{transform-box:fill-box;transform-origin:50% 100%;animation:flap .26s ease-in-out infinite alternate}
.park-map .wf{animation-delay:-.13s}
@keyframes flap{from{transform:scaleY(1)}to{transform:scaleY(-.35)}}
.park-map .bf{animation:flutter 11s ease-in-out infinite}
.park-map .bfly:nth-of-type(2n) .bf{animation-duration:14s;animation-direction:reverse}
...
.park-map .bw{transform-box:fill-box;transform-origin:100% 50%;animation:bwing .16s ease-in-out infinite alternate}
.park-map .bw2{transform-origin:0 50%}
@keyframes bwing{to{transform:scaleX(.25)}}
...
.flight .wing{transform-box:fill-box;transform-origin:50% 100%;animation:flap .24s ease-in-out infinite alternate}
""",
      js="",
...
st.addEventListener('mousemove',function(e){var r=st.getBoundingClientRect(),x=e.clientX-r.left,y=e.clientY-r.top;
...
st.addEventListener('click',function(e){if(e.target.closest('.again,a'))return;var r=st.getBoundingClientRect();
